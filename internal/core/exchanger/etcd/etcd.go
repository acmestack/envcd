...
//  @param etcdConfig
//  @return *Etcd
func New(etcdConfig *config.ConnMetadata) *Etcd {

	ctx := context.Background()

	if etcdConfig.Type != "etcd" {
		log.Printf("Scheme is not eq = %v", etcdConfig.Type)
		return nil
	}

	endpoint := etcdConfig.Host + ":" + etcdConfig.Port
	if endpoint == "" {
